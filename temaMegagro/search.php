<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" href="../img/logo.jpg" type="image/x-icon">
    <link rel="stylesheet" href="../css/index.css">
    <link rel="stylesheet" href="../css/bootstrap.min.css">
    <title>MegaAgropecuaria</title>
</head>
<body>
    <header>
        <nav class="navbar navbar-expand-lg">
            <div class="container-fluid d-flex justify-content-sm-center">
              <a class="navbar-brand" href="#">
                  <img src="../img/logo.jpg" alt="logo">
                  <b id="title">Megagropecuaria</b>
              </a>
                <input id="s" class="form-control me-2" type="search" placeholder="Buscar:" aria-label="Search">
              <button class="navbar-toggler ms-2" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
              </button>
              <div class="collapse navbar-collapse flex-column align-items-center justify-content-center" id="navbarSupportedContent">
                <ul class="navbar-nav mb-2 mb-lg-0 d-flex align-items-center justify-content-lg-start mt-3 mt-sm-0 justify-content-center">
                    <div class="d-flex justify-content-center">
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle me-3" href="#" id="categorias" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                Categorias
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="categorias">
                                <li><a class="dropdown-item" href="#">Medicamentos</a></li>
                                <li><a class="dropdown-item" href="#">Pecuaria</a></li>
                                <li><a class="dropdown-item" href="#">Agricola</a></li>
                                <li><a class="dropdown-item" href="#">Servicio</a></li>
                                <li><a class="dropdown-item" href="#">Agrotiendas</a></li>
                                <li><a class="dropdown-item" href="#">Vademécum</a></li>
                                <li><a class="dropdown-item" href="#">Mascotas</a></li>
                            </ul>
                        </li>
                        <li class="nav-item me-sm-3 me-0">
                            <a id="btnSesion" class="btn btn-outline-light" href="#">Iniciar Sesion</a>
                        </li>
                    </div>
                </ul>
              </div>
            </div>
          </nav>
    </header>
    <main id="search-content">
        <section class="mensaje-header mb-4">
            <span>Resultados de tu búsqueda:</span>
        </section>
        <section class="row px-5 w-100 m-0 d-flex justify-content-sm-between justify-content-center">
            <div class="col-lg-3 col-md-4 col-sm-5 col-12 d-flex flex-column px-sm-2 px-0">
                <span class="fw-bold fs-4">Búsqueda</span>
                <span>12 Resultados</span>
                <section class="aside-categories">
                    <span class="fw-bold fs-5">Categorias</span>
                    <ul class="mt-2">
                        <li><a href="#">Medicamentos</a></li>
                        <li><a href="#">Pecuaria</a></li>
                        <li><a href="#">Agricola</a></li>
                        <li><a href="#">Servicio</a></li>
                        <li><a href="#">Agrotiendas</a></li>
                        <li><a href="#">Vademécum</a></li>
                        <li><a href="#">Mascotas</a></li>
                    </ul>
                </section>
                <section class="aside-contact mb-sm-0 mb-3">
                    <img src="../img/logo.jpg" alt="logo">
                    <p class="text-center">¿Quieres saber más de nosotros?</p>
                    <a href="#" class="btn btn-outline-light">¡CONTÁCTANOS!</a>
                </section>
            </div>
            <div class="col-lg-9 col-md-8 col-sm-7 col-12 row px-md-4 px-0">
                <section class="row m-0 p-0 justify-content-center">
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product1.jpg" alt="">
                            <div>
                                <span class="precio">U$S 12.99</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product2.jpg" alt="">
                            <div>
                                <span class="precio">U$S 40.00</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4">
                        <a href="#">
                            <img src="../img/product3.jpg" alt="">
                            <div>
                                <span class="precio">U$S 25.45</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                </section>
                <section class="row m-0 p-0 justify-content-center">
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product4.jpg" alt="">
                            <div>
                                <span class="precio">U$S 5.99</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product5.jpg" alt="">
                            <div>
                                <span class="precio">U$S 10.00</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4">
                        <a href="#">
                            <img src="../img/product6.jpg" alt="">
                            <div>
                                <span class="precio">U$S 7.21</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                </section>
                <section class="row m-0 p-0 justify-content-center">
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product3.jpg" alt="">
                            <div>
                                <span class="precio">U$S 2.99</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4 me-sm-3 me-0">
                        <a href="#">
                            <img src="../img/product1.jpg" alt="">
                            <div>
                                <span class="precio">U$S 30.00</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                    <div class="col-4">
                        <a href="#">
                            <img src="../img/product2.jpg" alt="">
                            <div>
                                <span class="precio">U$S 15.45</span>
                                <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Veritatis.</p>
                            </div>
                        </a>
                    </div>
                </section>
            </div>
        </section>
    </main>
    <footer class="d-flex justify-content-end align-items-center mt-3">
        <a href="#">Términos y Condiciones</a>
        <a href="#">Quiénes Somos?</a>
        <a href="#">Contácto</a>
    </footer>
</body>
    <script src="../js/bootstrap.bundle.min.js"></script>
</html>